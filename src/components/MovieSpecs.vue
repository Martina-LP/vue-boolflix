        ...
        <div class="testo text-primary">
          ...
          <div class="language">
            <span>Lingua: 
              ...
              <span v-if="flagIcons.includes(film.original_language)">
                <img class="flag" :src="require(`../assets/img/${film.original_language}.png`)" alt="Flag">
              </span>
    ...
    <div class="col" v-for="tvShow in tvSeries" :key="tvShow.id">
      <div class="card bg-dark mt-4">
        <div class="testo text-primary">
          ...
          <div class="voto">
            <i class="fas fa-star text-warning"
              v-for="icon in Math.round(tvShow.vote_average / 2)"
              :key="icon.id"></i>
            <i class="far fa-star text-warning"
              v-for="icon in 5 - Math.round(tvShow.vote_average / 2)"
              :key="icon"></i>
          </div>
          <div class="overview">
            <p v-if="tvShow.overview.length > 180">
              Overview:
              {{ tvShow.overview.substring(-1, 300) + "..." }}
            </p>
            <p v-else>
              Overview:
              {{ tvShow.overview }}
